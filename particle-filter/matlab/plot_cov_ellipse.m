...
validateColor = @(x) isequal(size(x), [1, 1])&& ischar(x);
p = inputParser;
addRequired(p, 'mu');
...
a = sqrt(diag(D));
points = V * [cosd(thetas) * a(1); sind(thetas) * a(2)] + mu;
plot(points(1, :), points(2, :), p.Results.color, 'LineWidth', 2);
hold on
if (p.Results.showMean)
    plot(mu(1), mu(2), '*', 'MarkerSize', 15, 'color' , p.Results.color);
end
